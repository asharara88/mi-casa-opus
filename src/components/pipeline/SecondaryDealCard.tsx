...
import { Home, Calendar, AlertCircle, MoreHorizontal, FileCheck, CreditCard } from 'lucide-react';
...
interface SecondaryDealCardProps {
  deal: PipelineDeal;
  onClick: () => void;
  onSetNextAction: () => void;
}

export function SecondaryDealCard({
  deal,
  onClick,
  onSetNextAction,
}: SecondaryDealCardProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap gap-1 mb-2">
        ...
        {deal.mortgage_status && deal.mortgage_status !== 'not_applicable' && (
          <Badge variant="outline" className="text-xs">
            <CreditCard className="w-3 h-3 mr-1" />
            Mortgage: {deal.mortgage_status}
          </Badge>
        )}
      </div>

      {/* Transfer Info */}
      {deal.transfer_number && (
        <div className="text-xs text-muted-foreground mb-2">
          Transfer: {deal.transfer_number}
        </div>
      )}
